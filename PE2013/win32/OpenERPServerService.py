...
class PengERPServerService(win32serviceutil.ServiceFramework):
    # required info
    ...
    def SvcStop(self):
        ...
        # stop the running PengERP Server: say it's a normal exit
        win32api.TerminateProcess(int(self.terpprocess._handle), 0)
        servicemanager.LogInfoMsg("PengERP Server stopped correctly")
    ...
    def SvcDoRun(self):
        self.StartTERP()
        servicemanager.LogInfoMsg("PengERP Server up and running")
        # exit with same exit code as PengERP process
...
if __name__ == '__main__':
    ...
    win32serviceutil.HandleCommandLine(PengERPServerService, customOptionHandler=option_handler)
