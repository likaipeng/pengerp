...
The ``__init__.py`` file is the Python import file, because an PengERP module
...
All PengERP resources are objects: invoices, partners. Metadata are also object
...
The ORM of PengERP is constructed over PostgreSQL. It is thus possible to
query the object used by PengERP using the object interface (ORM) or by
...
..    *The Physical Objects Model of [PengERP version 3.0.3]*
...
General structure of PengERP XML files is more detailed in the 
...
PengERP objects using XML files. The general structure of an PengERP XML file
is as follows:
...
Let's review an example taken from the PengERP source (base_demo.xml in the base module):
...
PengERP uses a flexible and powerful reporting system. Reports are generated either in PDF or in HTML. Reports are designed on the principle of separation between the data layer and the presentation layer.
...
Each module has its own ``i18n`` folder. In addition, PengERP can now deal with
