...
==================================
Security in PengERP: users, groups
==================================

Users and user roles are critical points concerning internal security in
PengERP. PengERP provides several security mechanisms concerning user roles,
all implemented in the PengERP Server. They are implemented in the lowest
server level, which is the ORM engine. PengERP distinguishes three different
concepts:

 - user: a person identified by its login and password. Note that all employees
   of a company are not necessarily PengERP users; an user is somebody who
...
Users represent physical persons using PengERP. They are identified with
a login and a password,they use PengERP, they can edit their own preferences, ...
...
that suit all applications. Therefore, like SAP, PengERP is by default 
...
PengERP now supports real access control at the field level, not just on the view side.
...
As of PengERP 7.0 the existing behavior is preserved on the view level, but a new ``groups``
...
In PengERP, granting access to menus can be done using user groups. A menu 
...
Customizing views based on groups is possible in PengERP. You can put rules 
...
When installing your particular instance of PengERP, a specific first user 
...
menu, allowing the administration of PengERP.
...
accesses of every users of PengERP.
