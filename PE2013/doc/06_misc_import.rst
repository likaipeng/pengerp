...
PengERP has included a bulk import facility for CSV-ish files for a
...
field to a type matching what PengERP expects to write.
...
``field``
    the name of the (logical) PengERP field for which the error or
...
``moreinfo`` (optional)
    ...
    * If ``moreinfo`` is a dict, it is an PengERP action descriptor
