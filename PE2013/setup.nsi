...
!define PRODUCT_NAME "PengERP Server"
...
InstallDir "$PROGRAMFILES\PengERP ${VERSION}"
...
#VIAddVersionKey "LegalTrademark" "PengERP is a trademark of ${PUBLISHER}"
...
!define MUI_FINISHPAGE_LINK_LOCATION "http://www.pengerp.com/contact"
...
; English
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact PengERP for Partnership and/or Support"
...
; French
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez PengERP pour un Partenariat et/ou du Support"
...
Section PengERP_Server SectionPengERP_Server
    ...
    !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
        ...
        !insertmacro CreateInternetShortcut "$SMPROGRAMS\$STARTMENU_FOLDER\PengERP" "http://localhost:8069/"
...
# If there is a previous install of the PengERP Server, keep the login/password from the config file
    ...
    nsExec::Exec '"$INSTDIR\service\PengERPServerService.exe" -auto -install'
SectionEnd
...
;    WriteRegDWORD HKLM     "${UNINSTALL_REGISTRY_KEY}" "VersionMinor" "${REVISION_VERSION}"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@pengerp.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "http://www.pengerp.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@pengerp.com"
...
Section "Uninstall"
    ...
    ; Uninstall the PengERP Service
    nsExec::Exec '"$INSTDIR\..\service\PengERPServerService.exe" -remove'
    sleep 2
    ...
    Delete "$SMPROGRAMS\$MUI_TEMP\PengERP.url"
    ;
...
Function ShowPostgreSQL
    SectionGetFlags ${SectionPengERP_Server} $0
