...
def configure_parser():
    ...
    group = optparse.OptionGroup(parser, "Note",
        ...
        "with the PengERP server for case management in the CRM module.")
    parser.add_option_group(group)
    parser.add_option("-u", "--user", dest="userid",
                      help="PengERP user id to connect with",
                      default=config.OPENERP_DEFAULT_USER_ID, type='int')
    parser.add_option("-p", "--password", dest="password",
                      help="PengERP user password",
    ...
    parser.add_option("-d", "--dbname", dest="dbname",
                      help="PengERP database name (default: %default)",
                      default=config.OPENERP_DEFAULT_DATABASE)
    parser.add_option("--host", dest="host",
                      help="PengERP Server hostname",
                      default=config.OPENERP_HOSTNAME)
    parser.add_option("--port", dest="port",
                      help="PengERP Server XML-RPC port number",
...
def main():
    """
    Receive the email via the stdin and send it to the PengERP Server
    """
    ...
    try:
        ...
        sys.stderr.write("Failed to deliver email to PengERP Server, sending error notification to %s\n" % config.MAIL_ADMINS)
