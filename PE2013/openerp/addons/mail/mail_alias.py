...
#    Copyright (C) 2012 PengERP S.A. (<http://pengerp.com>).
#
...
class mail_alias(osv.Model):
    """A Mail Alias is a mapping of an email address with a given PengERP Document
       model. It is used by PengERP's mail gateway when processing incoming emails
       ...
       created, it becomes immediately usable and PengERP will accept email for it.
     """
    _name = 'mail.alias'
    ...
    _columns = {
        'alias_name': fields.char('Alias', required=True,
                            help="The name of the email alias, e.g. 'jobs' "
                                 "if you want to catch emails for <jobs@example.my.pengerp.com>",),
        'alias_model_id': fields.many2one('ir.model', 'Aliased Model', required=True, ondelete="cascade",
                                          help="The model (PengERP Document Kind) to which this alias "
    ...
    def name_get(self, cr, uid, ids, context=None):
        """Return the mail alias display alias_name, inclusing the implicit
           ...
           e.g. `jobs@openerp.my.pengerp.com` or `sales@openerp.my.pengerp.com`
        """
