...
#----------------------------------------------------------
# PengERPSession RPC openerp backend access
#----------------------------------------------------------
...
class PengERPSession(object):
    """
    An PengERP RPC session, a given user can own multiple such sessions
    in a web session.

    .. attribute:: context

        The session context, a ``dict``. Can be reloaded by calling
        :meth:`openerpweb.openerpweb.PengERPSession.get_context`
    ...
    def model(self, model):
        ...
        :param model: an PengERP model name
        :type model: str
    ...
    def _fix_lang(self, context):
        """ PengERP provides languages which may not make sense and/or may not
