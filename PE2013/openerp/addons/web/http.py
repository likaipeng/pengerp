...
class WebRequest(object):
    """ Parent class for all PengERP Web request types, mostly deals with
    ...
    .. attribute:: session_id
        ...
        opaque identifier for the :class:`session.PengERPSession` instance of
        the current request

    .. attribute:: session

        :class:`~session.PengERPSession` instance for the current request
...
class JsonRequest(WebRequest):
    ...
    def dispatch(self, method):
        ...
        try:
            ...
            error = {
                'code': 100,
                'message': "PengERP Session Invalid",
                'data': {
            ...
            error = {
                'code': 300,
                'message': "PengERP WebClient Error",
                'data': {
...
def session_context(request, session_store, session_lock, sid):
    ...
    try:
        ...
        # Remove all PengERPSession instances with no uid, they're generated
        # either by login process or by HTTP requests without an PengERP
        # session id, and are generally noise
        removed_sessions = set()
        for key, value in request.session.items():
            if not isinstance(value, session.PengERPSession):
                continue
        ...
        with session_lock:
            if sid:
                ...
                for k, v in request.session.iteritems():
                    ...
                    if stored and isinstance(v, session.PengERPSession):
...
class Root(object):
    """Root WSGI application for the PengERP Web Client.
    """
    def __init__(self):
        self.addons = {}
        self.statics = {}
