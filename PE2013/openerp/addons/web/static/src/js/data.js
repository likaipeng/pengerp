 ...
 * consumed by PengERP's RPC APIs.
 *
...
instance.web.Model = instance.web.Class.extend({
     ...
     *
     * @param {String} model_name name of the PengERP model this object is bound to
    ...
    /**
     * Call a method (over RPC) on the bound PengERP model.
     *
    ...
    /**
     * Executes a signal on the designated workflow, on the bound PengERP model
     *
...
instance.web.DataSet =  instance.web.Class.extend(instance.web.PropertiesMixin, {
    /**
     * Collection of PengERP records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the PengERP model this dataset will manage
     */
    ...
    get_id_index: function(id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // PengERP ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // PengERP ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, PengERP domain
     ...
     * an SQL-like form usable by PengERP's ORM.
     *
