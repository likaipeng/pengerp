...
Testing in PengERP Web
======================

Javascript Unit Testing
-----------------------

PengERP Web 7.0 includes means to unit-test both the core code of
PengERP Web and your own javascript modules. On the javascript side,
unit-testing is based on QUnit_ with a number of helpers and
extensions for better integration with PengERP.

To see what the runner looks like, find (or start) an PengERP server
with the web client enabled, and navigate to ``/web/tests`` e.g. `on
PengERP's CI <http://trunk.runbot.pengerp.com/web/tests>`_. This will
...
module. PengERP tests live in a :js:func:`~openerp.testing.section`,
...
actually does its job. PengERP Web test case use standard `QUnit
...
As noted above, PengERP Web's tests use `qunit assertions`_. They are
...
Getting an PengERP instance
---------------------------

The PengERP instance is the base through which most PengERP Web
...
.. js:function:: mock(rpc_spec, handler)
    ...
    * If it matches the pattern ``model:method`` (if it contains a
      ...
      call straight to the PengERP server (through XMLRPC) as
...
PengERP server. To do this, set the :js:attr:`rpc option
...
1. Install unittest2_ and QUnitSuite_ in your Python environment. Both
   ...
   The former is the unit-testing framework used by PengERP, the
...
3. Set up :ref:`PengERP Command <openerpcommand:pengerp.command>`,
   which will be used to actually run the tests: running the qunit_
   test suite requires a running server, so at this point PengERP
...
4. Install a new database with all relevant modules (all modules with
   ...
   .. note::
       ...
       being duplicated, but PengERP doesn't currently break
...
the PengERP Web test suite.

.. note::

    Note that this runs all the Python tests for the ``web`` module,
    but all the web tests for all of PengERP. This can be surprising.
