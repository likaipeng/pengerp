...
PengERP Web 7.0 implements a unified facets-based search view instead
...
makes significant use of Backbone's models and collections (PengERP
Web's widgets make a good replacement for Backbone's own views). As a
result, understanding the implementation details of the PengERP Web 7
...
The primary component of search views, as with all other PengERP
views, are inputs. The search view has two types of inputs — filters
and fields — but only one is easly customizable: fields.

The mapping from PengERP field types (and widgets) to search view
...
PengERP this is done via :ref:`domains <openerpserver:domains>`. On
the other hand, the PengERP Web 7 search view's state is modelled
...
:js:class:`~openerp.web.search.Facet` objects to PengERP search data.
...
of the peculiarities pertaining to PengERP's handling of fields in
...
PengERP Web 6.1 to PengERP Web 7.
