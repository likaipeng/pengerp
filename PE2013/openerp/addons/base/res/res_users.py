...
#    Copyright (C) 2010-2013 PengERP s.a. (<http://pengerp.com>).
#
...
class res_users(osv.osv):
    """ User class. A res.users record models an PengERP user and is different
        from an employee.
    ...
    _columns = {
        ...
        'user_email': fields.related('email', type='char',
            deprecated='Use the email field instead of user_email. This field will be removed with PengERP 7.1.'),
    }
    ...
    def unlink(self, cr, uid, ids, context=None):
        if 1 in ids:
            raise osv.except_osv(_('Can not remove root user!'), _('You can not remove the admin user as it is used internally for resources created by PengERP (updates, module installation, ...)'))
        db = cr.dbname
        if db in self._uid_cache:
            for id in ids:
                if id in self._uid_cache[db]:
