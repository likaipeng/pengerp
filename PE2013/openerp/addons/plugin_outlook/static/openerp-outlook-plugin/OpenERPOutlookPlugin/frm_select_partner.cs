...
namespace PengERPOutlookPlugin
{
    ...
    public partial class frm_select_partner : Form
    {
        ...
        private void btn_select_partner_select_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    int partner_id = (int)Cache.PengERPOutlookPlugin.CreatePartnerRecord(lstbox_select_partner.SelectedItem.ToString());
                    foreach (outlook.MailItem mailItem in Tools.MailItems())
                    {
                        Cache.PengERPOutlookPlugin.CreateContactRecord(partner_id, mailItem.SenderName, mailItem.SenderEmailAddress);
                    }
                    this.Close();
                }
            }
        ...
        public void search_lst_partner()
        {
            ...
            */
            try
            {
                ...
                foreach (Record oo in Cache.PengERPOutlookPlugin.SearchRecord(txt_select_partner.Text, "res.partner"))
                {
        ...
        {
            Record[] partenr_list = Cache.PengERPOutlookPlugin.SearchRecord(null,"res.partner");
