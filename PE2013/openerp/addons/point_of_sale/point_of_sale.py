...
class pos_config(osv.osv):
    ...
    _columns = {
        ...
        'iface_self_checkout' : fields.boolean('Self Checkout Mode',
             help="Check this if this point of sale should open by default in a self checkout mode. If unchecked, PengERP uses the normal cashier mode by default."),
        ...
        'sequence_id' : fields.many2one('ir.sequence', 'Order IDs Sequence', readonly=True,
            help="This sequence is automatically created by PengERP but you can change it "\
        ...
        for record in self.browse(cr, uid, ids, context=context):
            for st in record.statement_ids:
                ...
                if st.difference and st.journal_id.cash_control == True:
                    ...
                    if not account_id:
                        raise osv.except_osv( _('Error!'),
                        _("Please set your profit and loss accounts on your payment method '%s'. This will allow PengERP to post the difference of %.2f in your ending balance. To close this session, you can update the 'Closing Cash Control' to avoid any difference.") % (st.journal_id.name,st.difference))
                    bsl.create(cr, uid, {
                        'statement_id': st.id,
