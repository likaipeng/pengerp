...
#    Copyright (c) 2011-2012 PengERP S.A. <http://pengerp.com>
#
...
EDI_GENERATOR = 'PengERP ' + release.major_version
        ...
        for edi_document in edi_documents:
            ...
            if module != 'base' and not ir_module.search(cr, uid, [('name','=',module),('state','=','installed')]):
                raise osv.except_osv(_('Missing application.'),
                            _("The document you are trying to import requires the PengERP `%s` application. "
...
class EDIMixin(object):
    ...
    def _edi_external_id(self, cr, uid, record, existing_id=None, existing_module=None,
        ...
        refer to any record within any PengERP instance, without conflict.

        For PengERP records that have an existing "External ID" (i.e. an entry in
    ...
    def edi_metadata(self, cr, uid, records, context=None):
        ...
        The metadata fields always include::

            {
               ...
               '__generator' : 'PengERP',              # EDI generator
