...
class AddonsImportHook(object):
    """
    Import hook to load PengERP addons from multiple paths.

    PengERP implements its own import-hook to load its addons. PengERP
    ...
    When an PengERP addons name clashes with some other installed Python
    module (for instance this is the case of the `resource` addons),
    obtaining the PengERP addons is only possible with the long name. The
    ...
    def find_module(self, module_name, package_path):
        ...
        if len(module_parts) == 1 and \
            ...
            try:
                ...
                _logger.warning("""
Ambiguous import: the PengERP module `%s` is shadowed by another
...
def initialize_sys_path():
    """
    Setup an import-hook to be able to import PengERP addons from the different
    addons paths.
