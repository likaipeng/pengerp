...
"""
Miscellaneous tools used by PengERP.
"""
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    """Open a file from the PengERP root, using a subdir folder.
    ...
    if os.path.isabs(name):
        ...
        for root in adps + [rtp]:
            ...
            # It is outside the PengERP root: skip zipfile lookup.
...
def scan_languages():
    """ Returns all languages supported by PengERP for translation
...
def detect_server_timezone():
    ...
    sources = [ (config['timezone'], 'PengERP configuration'),
...
class upload_data_thread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            fp = urllib.urlopen('http://www.pengerp.com/scripts/survey.php', args)
            fp.read()
            fp.close()
        except Exception:
            pass
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns 
       ...
       the evaluation.
       Mostly useful for evaluating PengERP domains/contexts that
